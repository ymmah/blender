...
void BKE_scene_copy_data(Main *bmain, Scene *sce_dst, const Scene *sce_src, const int flag)
{
	     ...
	     scene_layer_src = scene_layer_src->next, scene_layer_dst = scene_layer_dst->next)
	{
		...
		BKE_freestyle_config_copy(&scene_layer_dst->freestyleConfig, &scene_layer_src->freestyleConfig, flag_subdata);
	...
	if (type == SCE_COPY_EMPTY) {
		...
		if (type == SCE_COPY_FULL) {
			...
			for (SceneLayer *scene_layer_dst = sce_copy->render_layers.first; scene_layer_dst; scene_layer_dst = scene_layer_dst->next) {
				for (FreestyleLineSet *lineset = scene_layer_dst->freestyleConfig.linesets.first; lineset; lineset = lineset->next) {
