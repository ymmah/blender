...
static int freestyle_module_add_exec(bContext *C, wmOperator *UNUSED(op))
{
	...
	BKE_freestyle_module_add(&scene_layer->freestyleConfig);
...
static int freestyle_module_remove_exec(bContext *C, wmOperator *UNUSED(op))
{
	...
	BKE_freestyle_module_delete(&scene_layer->freestyleConfig, module);
...
static int freestyle_module_move_exec(bContext *C, wmOperator *op)
{
	...
	if (BKE_freestyle_module_move(&scene_layer->freestyleConfig, module, dir)) {
		DEG_id_tag_update(&scene->id, 0);
		WM_event_add_notifier(C, NC_SCENE | ND_RENDER_OPTIONS, scene);
	}
...
static int freestyle_lineset_add_exec(bContext *C, wmOperator *UNUSED(op))
{
	...
	BKE_freestyle_lineset_add(bmain, &scene_layer->freestyleConfig, NULL);
...
static int freestyle_active_lineset_poll(bContext *C)
{
	...
	return BKE_freestyle_lineset_get_active(&scene_layer->freestyleConfig) != NULL;
}
...
static int freestyle_lineset_copy_exec(bContext *C, wmOperator *UNUSED(op))
{
	...
	FRS_copy_active_lineset(&scene_layer->freestyleConfig);
...
static int freestyle_lineset_paste_exec(bContext *C, wmOperator *UNUSED(op))
{
	...
	FRS_paste_active_lineset(&scene_layer->freestyleConfig);
...
static int freestyle_lineset_remove_exec(bContext *C, wmOperator *UNUSED(op))
{
	...
	FRS_delete_active_lineset(&scene_layer->freestyleConfig);
...
static int freestyle_lineset_move_exec(bContext *C, wmOperator *op)
{
	...
	if (FRS_move_active_lineset(&scene_layer->freestyleConfig, dir)) {
		DEG_id_tag_update(&scene->id, 0);
		WM_event_add_notifier(C, NC_SCENE | ND_RENDER_OPTIONS, scene);
	}
...
static int freestyle_linestyle_new_exec(bContext *C, wmOperator *op)
{
	...
	Scene *scene = CTX_data_scene(C);
	SceneLayer *scene_layer = BLI_findlink(&scene->render_layers, scene->active_layer);
	FreestyleLineSet *lineset = BKE_freestyle_lineset_get_active(&scene_layer->freestyleConfig);
