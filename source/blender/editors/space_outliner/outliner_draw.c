        ...
        const int num_elements)
{
	...
	UI_fontstyle_draw_simple(&fstyle_small,
	                         (offset_x + ufac + UI_UNIT_X * (0.12f - (num_digits - 1) * 0.12f)),
        ...
        const int num_elements)
{
	...
	tselem_draw_icon(block, xmax, (float)*offsx, (float)ys, tselem, te, 0.5f * alpha_fac, num_elements < 2);
	te->xs = *offsx;
	te->ys = ys;
...
static int tree_element_id_type_to_index(TreeElement *te)
{
	...
	if (id_index < INDEX_ID_OB) {
		...
		const Object *ob = (te->idcode == ID_OB) ? (Object *)tselem->id : NULL;
