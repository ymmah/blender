...
static void prepare(Render *re, SceneLayer *scene_layer)
{
	...
	// add style modules
	FreestyleConfig *config = &scene_layer->freestyleConfig;
...
static int displayed_layer_count(SceneLayer *scene_layer)
{
	...
	switch (scene_layer->freestyleConfig.mode) {
	case FREESTYLE_CONTROL_SCRIPT_MODE:
		for (FreestyleModuleConfig *module = (FreestyleModuleConfig *)scene_layer->freestyleConfig.modules.first;
		     module;
		     module = module->next)
		{
		...
		break;
	case FREESTYLE_CONTROL_EDITOR_MODE:
		for (FreestyleLineSet *lineset = (FreestyleLineSet *)scene_layer->freestyleConfig.linesets.first;
		     lineset;
		     lineset = lineset->next)
		{
			if (lineset->flags & FREESTYLE_LINESET_ENABLED)
...
Render *FRS_do_stroke_rendering(Render *re, SceneLayer *scene_layer, int render)
{
	...
	controller->setViewMapCache((scene_layer->freestyleConfig.flags & FREESTYLE_VIEW_MAP_CACHE) ? true : false);
